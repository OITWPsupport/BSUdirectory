  ...
  class pagination
  {
    ...
    function generate($array, $perPage = 10)
    {
      ...
      if (!empty($_GET['pagination'])) {
        $this->page = $_GET['pagination']; // using the get method+
      ...
      if (($this->pages) > 1)
      {
        ...
        if ($this->page != 1) {
          if ($this->showFirstAndLast) {
            $plinks[] = ' <a href="?pagination=1'.$queryURL.'">&laquo;&laquo; First </a> ';
          }
          $plinks[] = ' <a href="?pagination='.($this->page - 1).$queryURL.'">&laquo; Prev</a> ';
        }
        
        // Assign all the page numbers & links to the array
        for ($j = 1; $j < ($this->pages + 1); $j++) {
          if ($this->page == $j) {
            $links[] = ' <a class="selected" style=" font-weight:800">'.$j.'</a> '; // If we are on the same page as the current item
          } else {
            $links[] = ' <a href="?pagination='.$j.$queryURL.'">'.$j.'</a> '; // add the link to the array
          }
        }
  
        // Assign the 'next page' if we are not on the last page
        if ($this->page < $this->pages) {
          $slinks[] = ' <a href="?pagination='.($this->page + 1).$queryURL.'"> Next &raquo; </a> ';
          if ($this->showFirstAndLast) {
            $slinks[] = ' <a href="?pagination='.($this->pages).$queryURL.'"> Last &raquo;&raquo; </a> ';
          }
        }
